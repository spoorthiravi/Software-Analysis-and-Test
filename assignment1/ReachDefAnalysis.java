	...
	public void doAnalysis() {
		...
		while (updated) {
			for (BasicBlock b : cfg.reversePostOrder()) {
				...
				if(!b.isEntry())
				System.out.println("numOFpredecessors , predecessor = " + b.getNumberOfPredecessors() + "," + b.getPredecessors().get(0).getID());
				...
				for (int i = 0; i < b.size(); i++) {
					...
					System.out.println("definedregister size = " + definedRegisterOperandsList.size());
					...
					System.out.println("quadMetaList1 size = " + quadMetaList1.size());
					// killed Set for each quad; what if there is a loop to
					// first quad
					...
					if (i == 0 && b.isEntry()&& b.getNumberOfPredecessors() == 0) {
						...
						for (Register register : genSet) {
							...
							if (!linkedQuadMetaMap2.isEmpty()) {
								...
								/*QuadMetadata metadata = null;
								for(int j = 0; j < quadMetaList2.size();j++){
									System.out.println("quad id in list = " + quadMetaList2.get(j).quad.getID());
									System.out.println("current quad ID = " + q.getID());
									if(quadMetaList2.get(j).quad == q){
										metadata = quadMetaList2.get(j);
										break;
									}
								}*/
								...
								//QuadMetadata metadata = null;
								/*for(int j = 0; j < quadMetaList1.size();j++){
									if(quadMetaList1.get(j).quad == q){
										metadata = quadMetaList1.get(j);
					...
					if (b.isEntry() && i == 0) {
						...
						for (BasicBlock predecessor : predecessorsList) {
							...
							if(workListMap.containsKey(predecessor)){
								System.out.println("workListMap contains the predecessor");
								ArrayList<QuadMetadata> workListValues = workListMap.get(predecessor);
								if(workListValues.size() > 0){
									quadMeta.inSet.addAll((workListValues.get(workListValues.size() - 1)).outSet);
								}
							}else{
								System.out.println("workListMap does not contain the predecessor");
								continue;
							}
						}
					} else {
