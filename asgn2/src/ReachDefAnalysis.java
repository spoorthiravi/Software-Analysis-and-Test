...
public class ReachDefAnalysis extends DataflowAnalysis<Pair<Quad, Register>> {
	...
	static class BasicBlockMetadata {
		BasicBlock bb;
		private HashSet<RegisterOperand> genSet;
		private HashSet<RegisterOperand> killedSet;
		private HashSet<RegisterOperand> inSet;
		private HashSet<RegisterOperand> outSet;

		public BasicBlockMetadata(BasicBlock bb) {
			this.bb = bb;
			genSet = new HashSet<RegisterOperand>();
			killedSet = new HashSet<RegisterOperand>();
			inSet = new HashSet<RegisterOperand>();
			outSet = new HashSet<RegisterOperand>();
	...
	public void doAnalysis() {
		...
		while (updated) {
			for (BasicBlock b : cfg.reversePostOrder()) {
				ArrayList<HashSet<Register>> blockKillList = new ArrayList<HashSet<Register>>();
				ArrayList<HashSet<Register>> blockGenList = new ArrayList<HashSet<Register>>();
				for (int i = 0; i < b.size(); i++) {
					Quad q = b.getQuad(i);
					QuadMetadata quadMeta = new QuadMetadata(q);
					
					/** I want to put the inSet and outSet for each Quad in the induvidaualQuadMetaList so that
					 *  at the end of every loop I can check if the if the inSet for each Quad has changed if
					 *  it hasn't changed then I set updated to false so loop will break and then for each Quad 
					 *  I will take the last changed inSet and ouSet and put it in inMap and outMap respectively.
					 *  so I need only one induvidualQuadMetaList so I will check if its null and only if its null I'll create
					 *  it
					 */
					...
					System.out.println(definedRegisterOperandsList.size());
					...
					if(i == 0 && b.isEntry() && b.getNumberOfPredecessors() == 0){
						...
						for (Register register : genSet) {
							// should u remove all the registers in the genSet of previous quad that were generated in the genSet of this quad
							if(linkedQuadMetaMap2.containsKey(q)){
								if (quadMetaList2.get(quadMetaList2.indexOf(quadMeta) - 1).genSet.contains(register)) {
									killedSet.add(register);
								}
							}
							else{
								if (quadMetaList1.get(quadMetaList1.indexOf(quadMeta) - 1).genSet.contains(register)) {
									killedSet.add(register);
								}
							}
						}
					...
					if (b.isEntry() && i == 0) {
						...
						for (BasicBlock predecessor : predecessorsList) {
							ArrayList<QuadMetadata> workListValues = workListMap
									.get(predecessor);
							quadMeta.inSet.addAll((workListValues
									.get(workListValues.size() - 1)).outSet);
						}
					} else {
						quadMeta.inSet.addAll(quadMetaList1.get(quadMetaList1.indexOf(quadMeta) - 1).outSet);
					}
					...
					blockKillList.add(quadMeta.killedSet);
					blockGenList.add(quadMeta.genSet);
				}
			}
			boolean flag = true;
			...
			if(flag){
				...
				for(QuadMetadata quadMeta : quadMetaList1){
					...
					for (Register register : quadMeta.inSet) {
						Pair<Quad, Register> pair = new Pair<Quad, Register>(
							quadMeta.quad, register);
						inPairSet.add(pair);
					}
